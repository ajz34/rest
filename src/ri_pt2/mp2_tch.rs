mod mp2_tch {
    ...
    pub fn kernel_close_shell_mp2_gpu_tch(
        ...
        let d_vv_device = Tensor::from_slice(d_vv.as_slice().unwrap()).to_device(device).reshape([nvir as i64, nvir as i64]);
    
        let mut e_bi1 = Tensor::zeros([1], tch::kind::DOUBLE_CUDA);
        let mut e_bi2 = Tensor::zeros([1], tch::kind::DOUBLE_CUDA);
        ...
        for ptr_i in (0..nocc).step_by(batch_occ) {
            ...
            ).to_dtype_layout(tch::kind::FLOAT_CUDA, true, false).reshape([nbatch_i as i64, nvir as i64, naux as i64]);
            ...
            for ptr_j in (0..ptr_i+nbatch_i).step_by(batch_occ) {
                ...
                let cderi_ovu_batch_j = if (ptr_j == ptr_i) { &cderi_ovu_batch_i } else {
                    ...
                    ).to_dtype_layout(tch::kind::FLOAT_CUDA, true, false).reshape([nbatch_j as i64, nvir as i64, naux as i64])
                };
                tch::Cuda::synchronize(0);
                time_load += current.elapsed();
                ...
                for i in ptr_i .. ptr_i+nbatch_i {
                    for j in ptr_j .. std::cmp::min(ptr_j + nbatch_j, i + 1) {
                        let current = Instant::now();
                        let cderi_ivu = cderi_ovu_batch_i.select(0, (i - ptr_i) as i64);
                        let cderi_jvu = cderi_ovu_batch_j.select(0, (j - ptr_j) as i64);
                        let g_ab = cderi_ivu.matmul(&cderi_jvu.transpose(0, 1));
                        tch::Cuda::synchronize(0);
                        time_matmul += current.elapsed();
                        
                        let current = Instant::now();
                        let e_i = occ_energy[i] as f32;
                        let e_j = occ_energy[j] as f32;
                        let d_ab = e_i + e_j + &d_vv_device;
                        let t_ab = &g_ab / d_ab;
                        let factor = if i != j { 2.0 } else { 1.0 };
    
                        e_bi1 = e_bi1 + factor * (&g_ab * &t_ab).sum(tch::Kind::Float);
                        e_bi2 = e_bi2 + factor * (&g_ab * &t_ab.transpose(0, 1)).sum(tch::Kind::Float);
                        tch::Cuda::synchronize(0);
        ...
        let e_bi1 = e_bi1.double_value(&[]);
        let e_bi2 = e_bi2.double_value(&[]);
        let e_os = e_bi1;
    ...
    pub fn close_shell_pt2_tch(scf_data: &SCF) -> anyhow::Result<[f64; 3]> {
        ...
        if let Some(ri3mo_vec) = &scf_data.ri3mo {
            ...
            let result = kernel_close_shell_mp2_gpu_tch(cderi_ovu, occ_energy, vir_energy, Some(32))?;
            return Ok(result);
        } else {
...
mod debug_nh3 {
    ...
    fn test() {
        ...
        let result = close_shell_pt2_tch(&scf_data).unwrap();
        println!("{:?}", result);
...
mod debug_c20h42 {
    ...
    fn test() {
        ...
        let result = close_shell_pt2_tch(&scf_data).unwrap();
        println!("{:?}", result);
        println!("Elapsed time (MP2 tch): {:?}", start.elapsed());
        ...
        /*
            Elapsed time (RI3MO): 6.01348547s
            Total time: 4.219994278s
            Load time: 857.79593ms
            Matmul time: 2.102153148s
            Sum time: 996.576402ms
            [-3.7516944339134723, -2.93553973452023, -0.8161546993932425]
            Elapsed time (MP2 tch): 4.220010901s
            [-3.7516944921431614, -2.9355397848142903, -0.8161547073288709]
            Elapsed time (MP2 rayon): 12.632129817s
         */
    }
